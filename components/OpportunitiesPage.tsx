...
export default function OpportunitiesPage() {
  ...
  const handleExport = () => {
    const csv = [
      ['Title', 'Agency', 'Closing Date', 'Estimated Amount', 'Set-Aside Type'].join(','),
      ...opportunities.map((opp: any) =>
        [
          `"${opp.title}"`,
          opp.agency,
          new Date(opp.closing_date).toLocaleDateString(),
          opp.estimated_amount || 'N/A',
          opp.set_aside_type || 'N/A',
        ].join(',')
      ),
    ].join('\n');
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-slate-600 dark:text-slate-400">Loading opportunities...</p>
        </div>
      ) : viewMode === 'chart' ? (
